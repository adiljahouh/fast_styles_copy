...
class mst(op):
    ...
    def mst_net(self, x, style_control=None, reuse=False):
        with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=reuse):
            ...
            preds = tf.compat.v1.nn.tanh(x) * 150 + 255./2
        return preds
    ...
    def build_model(self):
        ...
        self.content_input = tf.compat.v1.placeholder(tf.float32, shape=[b, h, w, 3], name='content_input')
        ...
        style_input = tf.compat.v1.constant((style_img[np.newaxis, ...]), dtype=tf.float32)
        ...
        vgg_mean = tf.compat.v1.constant(np.array([103.939, 116.779, 123.68]).reshape((1, 1, 1, 3)), dtype='float32')
        ...
        if style_idx == 0:
            train_opt = tf.compat.v1.train.AdamOptimizer(self.learning_rate, self.momentum).minimize(loss)
        else:
            train_opt = tf.compat.v1.train.AdamOptimizer(self.learning_rate, self.momentum).minimize(loss, var_list=vars)

        self.optimize = [train_opt, loss, c_loss, s_loss, tv_loss]
        self.saver = tf.compat.v1.train.Saver(var_list=t_vars, max_to_keep=(self.max_to_keep))
